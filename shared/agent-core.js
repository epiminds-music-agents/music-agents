...
export function createAgent({
	...
	let currentMood =
		CYCLE_MOODS[Math.floor(Math.random() * CYCLE_MOODS.length)]
	...
	async function askAI(prompt, maxTokens = 200, options = {}) {
		if (!ai) {
			try {
				ai = createAI()
			} catch (err) {
	...
	function parsePlannedMoves(raw) {
		if (!raw) return []
		...
		const arrayStart = compact.indexOf("[")
		if (arrayStart !== -1) {
			let depth = 0
			for (let i = arrayStart; i < compact.length; i++) {
				const ch = compact[i]
				if (ch === "[") depth++
				else if (ch === "]") {
					depth--
					if (depth === 0) {
						candidates.push(compact.slice(arrayStart, i + 1))
						break
					}
				}
			}
		...
		for (const c of candidates) {
			try {
				...
				const normalized = parsed
					...
					.filter(
						(m) =>
							Number.isInteger(m.row) && Number.isInteger(m.step),
					)
					.filter((m) => m.row >= scope.start && m.row <= scope.end)
					.filter((m) => m.step >= 0 && m.step <= 15)
					.slice(0, 8)
...
BPM:${bpm} Rows:${scope.start}-${scope.end} Others:${
			otherAgents
...
Reply with 8 moves only. JSON array: [{"row":N,"step":N},...] rows ${scope.start}-${scope.end}, steps 0-15. No explanation.`
		...
		const result = await askAI(prompt, 180, {
		...
		const loop = () => {
			...
			if (now - lastToggle < interval) {
				loopTimer = setTimeout(
					loop,
					Math.max(20, interval - (now - lastToggle)),
				)
				return
			}
			...
			if (moveQueue.length > 0) {
				...
				lastToggle = Date.now()
				console.log(
					`[${name}] > r=${move.row} s=${move.step} (${moveQueue.length} left)`,
				)
			}
			...
			if (cycleCount >= cyclesUntilReset) {
				...
				currentMood =
					CYCLE_MOODS[Math.floor(Math.random() * CYCLE_MOODS.length)]
				console.log(
					`[${name}] Full reset in ${cyclesUntilReset} cycles, mood: ${currentMood}`,
				)
			...
			if (beatCounter % 120 === 0) {
				...
				chat(
					longContexts[
						Math.floor(Math.random() * longContexts.length)
					],
				).then((msg) => {
					if (msg) sendChat(msg)
				})
				...
				switch (msg.type) {
					...
					case "play_state":
						...
						if (isPlaying) {
							startPlayLoop()
							chat(
								"Playback just started. React in character—be hype, dry, mysterious, or absurd. One line.",
							).then((msg) => {
								if (msg) sendChat(msg)
							})
						} else {
					...
					case "scope_update": {
						...
						if (me) {
							...
							if (
								...
								moveQueue = []
								chat(
									`Your rows changed to ${scope.start}-${scope.end}. React in character—surprised, cool, sarcastic, or minimal. Vary your tone.`,
								).then((msg) => {
									if (msg) sendChat(msg)
								})
							}
